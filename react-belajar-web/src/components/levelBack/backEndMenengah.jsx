...
const BackEnduserMenengah = () => {
    const [materi, setMateri] = useState([]);

    useEffect(() => {
        axios.get("http://127.0.0.1:8000/materiback") // URL API Laravel
            .then((response) => {
                const materiMenengah = response.data.filter(materi => materi.level_materi === 'Manengah');
                setMateri(materiMenengah);
            })
            .catch((error) => {
        ...
        <div className="relative min-h-screen bg-gradient-to-br from-black via-[#0f172a] to-blue-800">
            ...
            <div className="relative z-20 container mx-auto max-w-full px-4">
                <div className='flex items-center justify-center h-20'>
                    <h1 className="text-2xl font-bold text-center text-white">Daftar Materi Back-End</h1>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-5">
                    {materi.map((item) => (
                        ...
                        >
                            <a href={`http://localhost:8000/storage/${item.link_materi}`}>
                                <img
...
export default BackEnduserMenengah;
